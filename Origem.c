...
GLuint loadTGA(const char* filename)
{
	...
	if (file == NULL)
	{
		printf("Arquivo de textura n�o encontrado!\n");
		return 0;
	}
	...
	{
		printf("Problema ao ler cabe�alho do arquivo!\n");
		fclose(file);
		return 0;
	}
	...
	{
		printf("Tipo de arquivo TGA inv�lido!\n");
		fclose(file);
		return 0;
	}
	...
	if ((width <= 0) || (height <= 0)){
		printf("Tamanho de arquivo inv�lido!\n");
		fclose(file);
		return 0;
	}
...
void displayPersp(void)
{
	...
	glPushMatrix();//inicio matrix terra
	...
	glPushMatrix();//inicio matrix lua
	glTranslatef(2, 0, 0);
	glRotatef(angleY, 0, 1, 0);
	glScalef(0.2, 0.2, 0.2);
	glBindTexture(GL_TEXTURE_2D, idLua);
	renderSphere();
	glPopMatrix();//fim matrix lua	
    glPopMatrix();//fim matrix terra

	glPushMatrix();//inicio matrix sol
	...
	renderSphere();
	glPopMatrix();//fim matrix sol	

	glPushMatrix();//inicio matrix jupter
	...
	renderSphere();
	glPopMatrix();//fim matrix jupter	


	glPushMatrix();//inicio matrix marte
